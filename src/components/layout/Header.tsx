...
const Header: React.FC = () => {
  ...
  return (
    <>
      ...
      <header
        className={`w-full sticky top-0 z-50 transform transition-transform duration-300 lg:block ${isHeaderVisible ? 'translate-y-0' : '-translate-y-full'
          }`}
      >
        <div className="bg-[#D81E05] shadow-md border-b border-[#A01A04]">
          <div className="container mx-auto px-4 py-3 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 md:gap-4">
              {/* Search Icon (Desktop only, already in search bar for desktop) */}
              ...
              {/* Cart Icon (Desktop only, now only visible on desktop here) */}
              <Button variant="ghost" size="icon" className="hidden lg:flex text-white hover:bg-[#A01A04]" asChild>
                <Link to="/cart">
              ...
              {/* User/Profile Icon with Dropdown (Desktop only, now only visible on desktop here) */}
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 z-50 lg:hidden shadow-lg">
        <div className="flex justify-around items-center h-14">
          ...
          {isLoggedIn ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex flex-col items-center justify-center text-[#222222] hover:text-[#D81E05] transition-colors text-xs font-medium gap-1 px-2 py-1">
                  <User className="h-5 w-5" />
                  <span>Account</span>
                </button>
              </DropdownMenuTrigger>
              <DropdownMenuContent side="top" align="center" className="w-48 bg-white text-[#222222] rounded-md shadow-lg z-50 border border-gray-200 mb-2">
                <DropdownMenuLabel className="text-[#D81E05]">Hi, {userName}</DropdownMenuLabel>
                <DropdownMenuSeparator className="bg-gray-200" />
                <DropdownMenuItem asChild>
                  <Link to="/account" className="flex items-center gap-2 cursor-pointer hover:bg-gray-100 py-2 px-2 rounded-md">
