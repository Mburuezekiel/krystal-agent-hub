...
const reviewProduct = asyncHandler(async (req, res) => {
  ...
  const product = await Product.findById(productId).populate('agent', 'email'); // Populate agent to get email for notification

  if (product) {
    if (status === 'approved' || status === 'rejected') {
      const oldStatus = product.reviewStatus; // Store old status for notification check

      product.reviewStatus = status;
      product.rejectionReason = status === 'rejected' ? reason : null;

      // If rejected, a reason is required
      ...
      // Send notification to agent only if status has changed
      if (oldStatus !== status) {
